...
try:
    ...
    if not file_path:
        messagebox.showerror(
            "Erro", "No file selected. Application will exit.")
        root.destroy()
        exit()
    try:
...
initial_text_elements_config = [
    ...
    ("text_added_time", "+9", 120, 67, 22, None,
    ...
    def _apply_action_and_update_ui(self, action_to_apply: EditAction, is_undo: bool):
        ...
        if hasattr(root, 'offsets_vars'):
            for key, var in root.offsets_vars.items():
                if var == action_to_apply.string_var:
                    update_value(key, action_to_apply.string_var,
                                 from_undo_redo=True)
                    is_offset_var = True
                    break
        if not is_offset_var and hasattr(root, 'color_vars'):
            for key, var in root.color_vars.items():
                if var == action_to_apply.string_var:
                    update_color_preview_from_entry(
                        key, action_to_apply.string_var, from_undo_redo=True)
                    break
        self.update_menu_states()
...
class BinaryReader:  # (Same)
    ...
    def read_int(self, c: int = 4, be: bool = False) -> int:
        if self.pos + c > len(self.data):
            raise ValueError(f"Not enough for int{c}")
        chunk = self.data[self.pos:self.pos+c]
        self.pos += c
...
class Decompressor:  # (Same)
    ...
    def decompress_eahd(data: bytes) -> bytes:
        try:
            ...
            logging.error(f"EAHD Decomp ValueError: {e}")
            return data
        except Exception as e:
            logging.error(f"EAHD Decomp Exception: {e}", exc_info=True)
            return data
...
class Compressor:  # (Same)
    ...
    def compress_eahd(data: bytes) -> bytes:
        logging.warning(
            "EAHD COMPRESSION IS NOT IMPLEMENTED. Ret uncompressed.")
        return data
    ...
    def _load(self):
        ...
        try:
            ...
            if magic not in (b'BIGF', b'BIG4'):
                raise ValueError(f"Invalid BIG magic: {magic}")
            reader.skip(4)
        ...
        for i in range(num_entries):
            ...
            if entry_raw_size == 0 and entry_name in {"sg1", "sg2"}:
                ...
                self.entries.append(FileEntry(
                    entry_offset, 0, entry_name, current_type_tag, Compression.NONE, b"", 0))
                continue
            ...
            if entry_offset + entry_raw_size > len(self.data_content):
                ...
                if actual_raw_size < 0:
                    actual_raw_size = 0
            raw_data = bytes(
                self.data_content[entry_offset: entry_offset + actual_raw_size])
            comp_type = Decompressor.detect_compression(raw_data)
            decomp_data = Decompressor.decompress_eahd(
                raw_data) if comp_type == Compression.EAHD else raw_data
            det_file_type = current_type_tag
            if decomp_data[:4] == b'DDS ':
                det_file_type = "DDS"
            self.entries.append(FileEntry(entry_offset, len(
                decomp_data), entry_name, det_file_type, comp_type, decomp_data, entry_raw_size))

    def list_files(
        self) -> List[str]: return [e.name for e in self.entries if e.size > 0]
...
def read_internal_name(fp: str) -> Optional[str]:  # (Same)
    ...
    try:
        ...
        logging.error(f"Read internal name fail: {e}")
        return None
...
def open_file():  # MODIFIED: No "Loaded" status, file_path_label updated
    ...
    if fp_temp:
        ...
        # Update persistent file path display
        ...
        if composite_mode_active:
            is_comp_eligible = (file_path and 0 <= current_image_index < len(
                image_files) and image_files[current_image_index] == "10")
            ...
            if is_internal_name_ok:
                ...
                current_image = None
                texture_label.config(text="Load .big / No internal name")
...
def extract_and_display_texture() -> bool:  # (Same)
    ...
    if not file_path:
        preview_canvas.delete("all")
        texture_label.config(text="No file")
        current_image = None
        return False
    try:
        ...
        if not entry or entry.size == 0 or not entry.data or entry.file_type != "DDS" or entry.data[:4] != b'DDS ':
            logging.info(f"Texture '{img_name}' invalid/not found/no data.")
            return False
        temp_dds_path = None
        try:
            ...
            bg_color = (255, 255, 255, 255) if preview_bg_color_is_white else (
                0, 0, 0, 255)
            pil_rgba = pil_img.convert(
                'RGBA') if pil_img.mode != 'RGBA' else pil_img
            ...
            logging.warning(f"Display DDS '{img_name}' failed: {e_display}")
            return False
        finally:
            if temp_dds_path and os.path.exists(temp_dds_path):
                os.remove(temp_dds_path)
    except Exception as e_outer:
        logging.error(f"Outer error in extract: {e_outer}", exc_info=True)
        return False
        ...
        with open(file_path, 'rb') as file:
            for off_tuple, var_obj in root.offsets_vars.items():
                ...
                try:
                    ...
                    if off_tuple in root.asterisk_labels:
                        root.asterisk_labels[off_tuple].config(text="")
                except Exception:
                    var_obj.set("ERR")
            for off_tuple, var_obj in root.color_vars.items():
                ...
                try:
                    ...
                    if off_tuple in root.color_previews:
                        root.color_previews[off_tuple].config(bg=hex_col)
                    if off_tuple in root.asterisk_labels:
                        root.asterisk_labels[off_tuple].config(text="")
                except Exception:
                    var_obj.set("#ERR")
    except Exception as e:
        messagebox.showerror("Error", f"Failed to read values: {e}")
...
def add_internal_name():  # (Same)
    ...
    if internal_name_str:
        ...
        if internal_name_str in offsets_data:
            ...
            offsets = {k: [int(str(v), 16) for v in (vl if isinstance(vl, list) else [
                vl])] for k, vl in config.get("offsets", {}).items()}
            colors = {k: [int(str(v), 16) for v in (vl if isinstance(vl, list) else [
                vl])] for k, vl in config.get("colors", {}).items()}
            recreate_widgets()
            load_current_values()
            previous_file_path = file_path
        else:
            messagebox.showerror(
                "Config Error", f"Config for '{internal_name_str}' not found.")
            internal_name_label.config(
                text=f"Internal Name: {internal_name_str} (No Config)")
            clear_editor_widgets()
            current_reference_width = None
            current_reference_height = None
            if composite_mode_active:
        ...
        if previous_file_path:
            ...
            add_internal_name()
            update_status(
                f"Reverted to: {os.path.basename(file_path)}", "orange")
        else:
...
def recreate_widgets():  # (Same, includes root.offset_entry_widgets, root.asterisk_labels)
    ...
    root.color_vars = {tuple(v): tk.StringVar(value='#000000')
    ...
    def make_offset_update_lambda(key_tuple, var, entry_widget):
        def on_offset_update(event=None):
            ...
            if old_value != new_value and not is_focus_out_after_key_release:
                # Don't record for FocusOut if it's the same value as after KeyRelease
                if not (event and event.type == tk.EventType.FocusOut):
                    action = EditAction(
                        var, old_value, new_value, key_tuple, entry_widget, f"Offset change for {key_tuple}")
    ...
    def make_increment_lambda(key_tuple, var, entry_widget, direction):
        def on_increment(event=None):
            ...
            if old_value != new_value:
                action = EditAction(
                    var, old_value, new_value, key_tuple, entry_widget, f"Increment {direction}")
                undo_manager.record_action(action)
            update_value(key_tuple, var)
            entry_widget.bind("<KeyRelease>", make_offset_update_lambda(
                key_tuple, var, entry_widget))
        return on_increment
    ...
    for lbl, off_list in offsets.items():
        ...
        if "Size" not in lbl and not lbl.startswith("Image_"):
            ...
            base_col = col
            tk.Label(positions_frame, text=lbl).grid(
                row=row_p, column=base_col, padx=5, pady=5, sticky="w")
            entry = tk.Entry(
                positions_frame, textvariable=root.offsets_vars[key_tuple], width=10)
            entry.grid(row=row_p, column=base_col+1, padx=0, pady=5)
            update_lambda = make_offset_update_lambda(
                key_tuple, root.offsets_vars[key_tuple], entry)
            entry.bind("<KeyRelease>", update_lambda)
            entry.bind("<FocusOut>", update_lambda)
            entry.bind('<KeyPress-Up>', make_increment_lambda(key_tuple,
                       root.offsets_vars[key_tuple], entry, "Up"))
            entry.bind('<KeyPress-Down>', make_increment_lambda(key_tuple,
                       root.offsets_vars[key_tuple], entry, "Down"))
            asterisk_lbl = tk.Label(
                positions_frame, text="", fg="red", width=1)
            asterisk_lbl.grid(row=row_p, column=base_col+2,
                              padx=(0, 5), pady=5, sticky="w")
            root.asterisk_labels[key_tuple] = asterisk_lbl
            root.offset_entry_widgets[key_tuple] = entry
            if col == 4 or "Y" in lbl or "Height" in lbl:
                row_p += 1
    row_s = 0
    for lbl, off_list in offsets.items():
        key_tuple = tuple(off_list)
        if "Size" in lbl:
            tk.Label(sizes_frame, text=lbl).grid(
                row=row_s, column=0, padx=5, pady=5, sticky="w")
            entry = tk.Entry(
                sizes_frame, textvariable=root.offsets_vars[key_tuple], width=10)
            entry.grid(row=row_s, column=1, padx=0, pady=5)
            update_lambda_size = make_offset_update_lambda(
                key_tuple, root.offsets_vars[key_tuple], entry)
            ...
            entry.bind('<KeyPress-Down>', make_increment_lambda(key_tuple,
                       root.offsets_vars[key_tuple], entry, "Down"))
            asterisk_lbl_size = tk.Label(
                sizes_frame, text="", fg="red", width=1)
            asterisk_lbl_size.grid(row=row_s, column=2,
                                   padx=(0, 5), pady=5, sticky="w")
    ...
    for lbl, off_list in colors.items():
        key_tuple = tuple(off_list)
        tk.Label(colors_frame, text=lbl).grid(
            row=row_c, column=0, padx=5, pady=5, sticky="w")
        entry = tk.Entry(
            colors_frame, textvariable=root.color_vars[key_tuple], width=10)
        ...
        def make_color_update_lambda(k_t, var, entry_w):
            def on_color_update(event=None):
                ...
                new_val = var.get()
                is_focus_out_after_key = (event and event.type == tk.EventType.FocusOut and hasattr(
                    entry_w, '_undo_recorded_for_this_change') and entry_w._undo_recorded_for_this_change)
                if old_val != new_val and not is_focus_out_after_key:
                    if not (event and event.type == tk.EventType.FocusOut):
                        action = EditAction(
                            var, old_val, new_val, k_t, entry_w, f"Color change for {k_t}")
                        undo_manager.record_action(action)
                    if event and event.type == tk.EventType.KeyRelease:
                        entry_w._undo_recorded_for_this_change = True
                update_color_preview_from_entry(k_t, var)
                if event and event.type == tk.EventType.FocusOut:
                    if hasattr(entry_w, '_undo_recorded_for_this_change'):
                        delattr(entry_w, '_undo_recorded_for_this_change')
            return on_color_update
        color_update_lambda = make_color_update_lambda(
            key_tuple, root.color_vars[key_tuple], entry)
        entry.bind('<KeyPress>', lambda e,
                   v=root.color_vars[key_tuple]: restrict_color_entry(e, v))
        entry.bind('<KeyRelease>', color_update_lambda)
        entry.bind("<FocusOut>", color_update_lambda)
        preview_lbl = tk.Label(colors_frame, bg=root.color_vars[key_tuple].get(
        ), width=3, height=1, relief="sunken")
        ...
        def make_choose_color_lambda(k_t, var_obj, preview_widget, entry_ref):
            def on_choose_color(event=None):
                ...
                if old_color != new_color:
                    action = EditAction(
                        var_obj, old_color, new_color, k_t, entry_ref, f"Choose color for {k_t}")
                    undo_manager.record_action(action)
                update_color_preview_from_entry(k_t, var_obj)
            return on_choose_color
        preview_lbl.bind("<Button-1>", make_choose_color_lambda(key_tuple,
                         root.color_vars[key_tuple], preview_lbl, entry))
        root.color_previews[key_tuple] = preview_lbl
        asterisk_lbl_color = tk.Label(colors_frame, text="", fg="red", width=1)
        asterisk_lbl_color.grid(row=row_c, column=3,
                                padx=(0, 5), pady=5, sticky="w")
...
def save_file():  # MODIFIED to update original_loaded values and clear asterisks
    ...
    if not hasattr(root, 'offsets_vars') or not hasattr(root, 'color_vars'):
        messagebox.showerror("Error", "Data not init.")
        return
    try:
        with open(file_path, 'r+b') as f:
            for off_key, var_obj in root.offsets_vars.items():
                ...
                try:
                    ...
                    messagebox.showerror(
                        "Save Err", f"Bad float '{val_str}' for {off_key}")
                    return
                for addr in off_key:
                    f.seek(addr)
                    f.write(packed)
            ...
            for off_key, var_obj in root.color_vars.items():
                ...
                try:
                    ...
                    messagebox.showerror(
                        "Save Err", f"Bad hex '{hex_str}' for {off_key}")
                    return
                for addr in off_key:
                    f.seek(addr)
                    f.write(bgra)
        ...
        messagebox.showerror("Save Err", f"Open/Save fail: {e}")


# MODIFIED to update asterisk
def update_value(offset_key_tuple, string_var, from_undo_redo=False):
    ...
    try:
        ...
        if not from_undo_redo:
            update_status(f"Invalid float '{val_str}'", "red")
        if offset_key_tuple in root.asterisk_labels:
            root.asterisk_labels[offset_key_tuple].config(text="!")
        return
    if offset_key_tuple in root.asterisk_labels:
        ...
        if original_val is not None:
            try:
                if abs(float(original_val) - new_game_offset_val) < 0.0001:
                    is_changed = False
            except ValueError:
                pass
        root.asterisk_labels[offset_key_tuple].config(
            text="*" if is_changed else "")
    if not from_undo_redo:
        update_status(
            f"Game offset for {offset_key_tuple} now {new_game_offset_val:.2f}", "blue")
    ...
    if composite_mode_active:  # Logic to update composite view if this offset is linked
        ...
        for el_data in composite_elements:
            ...
            if x_label and x_label in offsets and tuple(offsets[x_label]) == offset_key_tuple:
                is_x_offset = True
            y_label = el_data.get('y_offset_label_linked')
            if y_label and y_label in offsets and tuple(offsets[y_label]) == offset_key_tuple:
                is_y_offset = True

            if is_x_offset or is_y_offset:
                gui_ref_x, gui_ref_y = el_data.get(
                    'gui_ref_x'), el_data.get('gui_ref_y')
                base_game_x, base_game_y = el_data.get(
                    'base_game_x'), el_data.get('base_game_y')

                if gui_ref_x is not None and gui_ref_y is not None:
                    if is_x_offset and base_game_x is not None:
                        el_data['original_x'] = float(
                            gui_ref_x) + (new_game_offset_val - base_game_x)
                        visual_updated_for_element = el_data
                    if is_y_offset and base_game_y is not None:
                        el_data['original_y'] = float(
                            gui_ref_y) + (new_game_offset_val - base_game_y)
                        visual_updated_for_element = el_data

                if visual_updated_for_element:
                    logging.info(
                        f"Comp: Visual for {el_data.get('display_tag')} updated from Entry to ({el_data['original_x']:.1f}, {el_data['original_y']:.1f})")
                    ...
                    if leader_tag:  # If this element is a leader, update its followers
                        for follower_elem in composite_elements:
                            if follower_elem.get('conjoined_to_tag') == leader_tag:
                                follower_elem['original_x'] = visual_updated_for_element['original_x'] + \
                                    follower_elem.get('relative_offset_x', 0)
                                follower_elem['original_y'] = visual_updated_for_element['original_y'] + \
                                    follower_elem.get('relative_offset_y', 0)
                    break
        if visual_updated_for_element:
            redraw_composite_view()


# (No change from previous, calls update_value)
...
# Added from_undo_redo, updates asterisk
def update_color_preview_from_entry(off_key, str_var, from_undo_redo=False):
    ...
    if len(hex_str) == 7 and hex_str.startswith('#'):
        try:
            ...
            if not from_undo_redo:
                update_status("Bad hex color", "red")
        ...
        str_var.set("#"+hex_str)
        update_color_preview_from_entry(off_key, str_var, from_undo_redo)
    if off_key in root.asterisk_labels:
...
# (Same - already handles undo and calls update_color_preview_from_entry)
def choose_color(off_key, str_var, preview_widget):
    old_color = str_var.get()
    curr_hex = old_color
    if not (curr_hex.startswith("#") and len(curr_hex) == 7):
        curr_hex = "#000000"
    ...
    if new_color_tuple and new_color_tuple[1]:
        ...
        # Check if this key is for an offset entry (unlikely for color)
        if hasattr(root, 'offset_entry_widgets'):
            entry_widget_for_color = root.offset_entry_widgets.get(off_key)
        if old_color != chosen_hex:
            action = EditAction(str_var, old_color, chosen_hex, off_key,
                                entry_widget_for_color, f"Choose color for {off_key}")
            undo_manager.record_action(action)
...
def update_status(msg, fg_col): status_label.config(
    text=msg, fg=fg_col)  # (Same)
...
def about():  # (Same)
    ...
    tk.Label(win, text="FLP Scoreboard Editor 25 By FIFA Legacy Project.",
             pady=10, font=("Helvetica", 12, "bold")).pack()
    tk.Label(win, text="Version 1.13 [Build 10 May 2025]", pady=10).pack()
    tk.Label(
        win, text="© 2025 FIFA Legacy Project. All Rights Reserved.", pady=10).pack()
    tk.Label(win, text="Designed & Developed By Emran_Ahm3d.", pady=10).pack()
    tk.Label(win, text="Special Thanks to Riesscar, KO, MCK and Marconis for the Research.",
             pady=10, wraplength=400).pack()
    tk.Label(win, text="Discord: @emran_ahm3d", pady=10).pack()


def show_documentation(): webbrowser.open(
    "https://soccergaming.com/")  # (Same)


def restrict_color_entry(event, str_var):  # (Same)
    if event.char and event.char not in "#0123456789abcdefABCDEF" and len(str_var.get()) == 0 and event.char != '#':
        return 'break'
    if len(str_var.get()) >= 7 and event.char and event.widget.select_present() == 0:
        return 'break'
...
def previous_image():  # (Same - smarter browsing)
    ...
    if not extract_and_display_texture():
        preview_canvas.delete("all")
        texture_label.config(text="No displayable textures")
        current_image = None
...
# MODIFIED: No "Switched to 10.dds" status message. Initial pan.
def toggle_composite_mode():
    ...
    logging.info(
        f"Toggling composite mode. Current state: {composite_mode_active}")
    ...
    if not composite_mode_active:
        ...
        if not file_path or not is_internal_name_ok:
            messagebox.showinfo(
                "Info", "Load a .big file with a valid internal name config first.")
            logging.warning(
                "Prerequisites for composite mode not met (file/internal name).")
            return
        ...
        if not (0 <= current_image_index < len(image_files) and image_files[current_image_index] == "10"):
            ...
            try:
                ...
                if not extract_and_display_texture():
                    messagebox.showerror(
                        "Error", "Could not display 10.dds. Cannot enter composite mode.")
                    logging.error(
                        "Failed to display 10.dds for composite mode entry.")
                    return
                # update_status("Switched to 10.dds for composite mode", "blue") # Removed status
                root.update_idletasks()
            except ValueError:
                messagebox.showerror(
                    "Error", "'10' not found in image_files. Cannot enter composite.")
                return
        ...
        if not composite_mode_active:
            logging.warning("display_composite_view failed.")
            return

        composite_view_button.config(text="Single View")
        left_arrow_button.pack_forget()
        ...
        composite_mode_active = False
        clear_composite_view()
        composite_view_button.config(text="Composite View")
        left_arrow_button.pack(side=tk.LEFT, padx=(0, 5),
...
def clear_composite_view():  # (Same)
    ...
    for el in composite_elements:
        if 'tk_image_ref' in el and el['tk_image_ref']:
            del el['tk_image_ref']
    composite_elements.clear()
    preview_canvas.config(bg="#CCCCCC")
...
def redraw_composite_view():  # (Same - handles dynamic text color)
    ...
    for el_data in composite_elements:
        el_data['current_x'] = view_origin_x + \
            el_data['original_x'] * composite_zoom_level
        el_data['current_y'] = view_origin_y + \
            el_data['original_y'] * composite_zoom_level
        draw_x = el_data['current_x']
        draw_y = el_data['current_y']
        if el_data.get('type') == "text":
            base_font_size = el_data.get(
                'base_font_size', DEFAULT_TEXT_BASE_FONT_SIZE)
            zoomed_font_size = max(
                1, int(base_font_size * composite_zoom_level))
            ...
            if color_label and color_label in colors and hasattr(root, 'color_vars'):
                ...
                if color_key_tuple in root.color_vars:
                    ...
                    if current_hex and current_hex.startswith("#") and len(current_hex) == 7:
                        ...
                        logging.warning(
                            f"Invalid hex '{current_hex}' for '{color_label}'. Using fallback.")
                else:
                    logging.warning(
                        f"Var for key {color_key_tuple} (label: {color_label}) not in root.color_vars.")
            elif color_label:
                logging.warning(
                    f"Color label '{color_label}' not found in global 'colors' dict.")
            item_id = preview_canvas.create_text(int(draw_x), int(draw_y), anchor=tk.NW,
                                                 text=el_data['text_content'],
                                                 font=(
                                                     font_family, zoomed_font_size, "bold"),
                                                 fill=text_color,
                                                 tags=("composite_item", el_data['display_tag']))
            el_data['canvas_id'] = item_id
        elif el_data.get('type') == "image":
            pil_img = el_data['pil_image']
            zoomed_w = int(pil_img.width * composite_zoom_level)
            zoomed_h = int(pil_img.height * composite_zoom_level)
            if zoomed_w <= 0 or zoomed_h <= 0:
                continue
            try:
                ...
                logging.error(
                    f"Redraw img {el_data.get('display_tag')}: {e_redraw_img}")
        else:
            logging.warning(f"Unknown element type: {el_data.get('type')}")


# MODIFIED: Initial visual positions from GUI_Ref + Deviation.
def display_composite_view():
    ...
    logging.info(
        "Displaying composite view (Visuals relative to game data deviation).")
    if not file_path:
        composite_mode_active = False
        return
    ...
    try:
        big_file = FifaBigFile(file_path)
        if not read_internal_name(file_path):
            composite_mode_active = False
            return
        canvas_w = preview_canvas.winfo_width()
        canvas_h = preview_canvas.winfo_height()
        ...
        images_to_load_config = [
            ("10", "10"), ("14", "14"), ("30", "30_orig"), ("30", "30_dup")]
        source_entries_map = {e.name: e for e in big_file.entries if e.name in [
            "10", "14", "30"] and e.file_type == "DDS" and e.data}

        temp_composite_elements_dict: Dict[str, Dict[str, Any]] = {}

        for name_to_find, display_suffix in images_to_load_config:
            if name_to_find not in source_entries_map:
                continue
            entry = source_entries_map[name_to_find]
            temp_dds_path = None
            try:
                with tempfile.NamedTemporaryFile(delete=False, suffix=".dds") as tmp_f:
                    tmp_f.write(entry.data)
                ...
                img_config_tuple = predefined_image_coords.get(
                    current_display_tag)
                if not img_config_tuple:
                    logging.error(
                        f"Missing predefined_image_coords for {current_display_tag}. Skipping.")
                    continue
                ...
                img_x_var_linked = None
                img_y_var_linked = None
                ...
                if img_x_offset_label and img_y_offset_label and base_game_img_x is not None and base_game_img_y is not None and \
                   hasattr(root, 'offsets_vars') and offsets:
                    if img_x_offset_label in offsets and img_y_offset_label in offsets:
                        x_key = tuple(offsets[img_x_offset_label])
                        y_key = tuple(offsets[img_y_offset_label])
                        if x_key in root.offsets_vars and y_key in root.offsets_vars:
                            img_x_var_linked = root.offsets_vars[x_key]
                            img_y_var_linked = root.offsets_vars[y_key]
                            try:
                                ...
                                current_original_x = float(
                                    gui_ref_x) + deviation_x
                                current_original_y = float(
                                    gui_ref_y) + deviation_y
                            except ValueError:
                                img_x_var_linked = None
                                img_y_var_linked = None
                ...
                logging.error(
                    f"Error prep img {entry.name}({display_suffix}): {e_img}")
            finally:
                if temp_dds_path and os.path.exists(temp_dds_path):
                    os.remove(temp_dds_path)
        ...
        leader_tag_for_img14 = "text_added_time"
        follower_tag_img14 = "img_14"
        if leader_tag_for_img14 in temp_composite_elements_dict and follower_tag_img14 in temp_composite_elements_dict:
            ...
            text_added_time_cfg = next(
                item for item in initial_text_elements_config if item[0] == leader_tag_for_img14)
            ...
            follower_el['original_x'] = leader_el['original_x'] + \
                design_relative_offset_x
            follower_el['original_y'] = leader_el['original_y'] + \
                design_relative_offset_y
...
def zoom_composite_view(event):  # (Same)
    ...
    if not composite_elements:
        return
    factor = 1.1 if event.delta > 0 else (1/1.1)
    new_zoom = max(0.05, min(composite_zoom_level * factor, 10.0))
    canvas_w = preview_canvas.winfo_width()
    canvas_h = preview_canvas.winfo_height()
    mouse_x_rel_center = event.x - canvas_w / 2
    mouse_y_rel_center = event.y - canvas_h / 2
    if abs(composite_zoom_level) > 1e-6 and abs(new_zoom) > 1e-6:
        composite_pan_offset_x += mouse_x_rel_center * \
            (1.0/composite_zoom_level - 1.0/new_zoom)
        composite_pan_offset_y += mouse_y_rel_center * \
            (1.0/composite_zoom_level - 1.0/new_zoom)
...
def start_pan_composite(event):  # (Same)
    ...
    drag_data["y"] = event.y
    logging.debug(
        f"start_pan_composite: RMB PANNING started at ({event.x},{event.y})")
    ...
    for el_data in composite_elements:
        if el_data.get('canvas_id') == item_id:
            if el_data.get('is_fixed', False):
                logging.info(
                    f"Attempted to drag fixed item: {el_data.get('display_tag')}")
                composite_drag_data['item'] = None
                return
            ...
            if x_var and y_var:
                try:
                    initial_game_x = float(x_var.get())
                    initial_game_y = float(y_var.get())
                except ValueError:
                    logging.warning(
                        f"Could not parse initial game offsets for {el_data.get('display_tag')}. Using 0,0.")
            ...
            if x_label and x_label in offsets and hasattr(root, 'offset_entry_widgets'):
                ...
                if x_key in root.offset_entry_widgets:
                    ...
                    highlighted_offset_entries.append(
                        (entry_w, entry_w.cget("background")))
                    entry_w.config(bg="lightyellow")
...
# (Same - already handles relative game offset updates 1:1)
def on_drag_composite(event):
    global composite_drag_data, composite_zoom_level, drag_data, composite_elements, offsets
    if event.num == 3 or drag_data.get("is_panning_rmb"):
        on_pan_composite(event)
        return
    if composite_drag_data.get('item') is None or drag_data.get("is_panning"):
        return
    dragged_elem_data = composite_drag_data.get('element_data')
    if not dragged_elem_data:
        return
    ...
    new_visual_original_x = composite_drag_data['start_original_x'] + \
        delta_visual_original_x
    new_visual_original_y = composite_drag_data['start_original_y'] + \
        delta_visual_original_y
    ...
    if x_var_linked and y_var_linked:
        ...
        old_x_str = x_var_linked.get()
        old_y_str = y_var_linked.get()
        x_var_linked.set(f"{new_game_offset_x:.2f}")
        y_var_linked.set(f"{new_game_offset_y:.2f}")
        logging.info(
            f"  -> For '{dragged_elem_data.get('display_tag')}': Linked Game Offsets set to X={new_game_offset_x:.2f}, Y={new_game_offset_y:.2f} (relative to start)")
        ...
        if x_offset_label and x_offset_label in offsets:
            ...
            if old_x_str != x_var_linked.get():
                action_x = EditAction(x_var_linked, old_x_str, x_var_linked.get(), x_key_tuple, root.offset_entry_widgets.get(
                    x_key_tuple), f"Drag {dragged_elem_data.get('display_tag')} X")
        ...
        if y_offset_label and y_offset_label in offsets:
            ...
            if old_y_str != y_var_linked.get():
                action_y = EditAction(y_var_linked, old_y_str, y_var_linked.get(), y_key_tuple, root.offset_entry_widgets.get(
                    y_key_tuple), f"Drag {dragged_elem_data.get('display_tag')} Y")
                undo_manager.record_action(action_y)
            update_value(y_key_tuple, y_var_linked, from_undo_redo=True)
    if dragged_elem_data.get('type') == "text":
        logging.info(
            f"Text '{dragged_elem_data.get('display_tag')}' {log_message_suffix}")
    elif dragged_elem_data.get('type') == "image":
        logging.info(
            f"Image '{dragged_elem_data.get('display_tag')}' {log_message_suffix}")
    dragged_tag = dragged_elem_data.get('display_tag')
    if dragged_tag:
        for follower_elem in composite_elements:
            if follower_elem.get('conjoined_to_tag') == dragged_tag:
                follower_elem['original_x'] = dragged_elem_data['original_x'] + \
                    follower_elem.get('relative_offset_x', 0)
                follower_elem['original_y'] = dragged_elem_data['original_y'] + \
                    follower_elem.get('relative_offset_y', 0)
    redraw_composite_view()
...
def on_drag_release_composite(event):  # Same
    ...
    if event.num == 3:
        drag_data["is_panning_rmb"] = False
        logging.debug("RMB Pan released")
...
editmenu.add_command(label="Undo", command=lambda: undo_manager.undo(
), accelerator="Ctrl+Z", state=tk.DISABLED)
editmenu.add_command(label="Redo", command=lambda: undo_manager.redo(
), accelerator="Ctrl+Y", state=tk.DISABLED)
menubar.add_cascade(label="Edit", menu=editmenu)
root.editmenu = editmenu
...
positions_scrollbar = ttk.Scrollbar(
    positions_frame_container, orient="vertical", command=positions_canvas.yview)
...
left_arrow_button = ttk.Button(
    preview_controls_frame, text="◀", command=previous_image, width=2)
left_arrow_button.pack(side=tk.LEFT, padx=(0, 5), pady=5, anchor='center')
preview_canvas = tk.Canvas(preview_controls_frame, width=580,
                           height=150, bg="#CCCCCC", relief="solid", bd=1)
preview_canvas.pack(side=tk.LEFT, padx=5, pady=5, anchor='center')
preview_canvas.bind("<MouseWheel>", zoom_image_handler)
preview_canvas.bind("<ButtonPress-1>", start_drag_handler)
preview_canvas.bind("<B1-Motion>", on_drag_handler)
preview_canvas.bind("<ButtonRelease-1>", on_drag_release_handler)
preview_canvas.bind("<ButtonPress-3>", start_pan_composite)
preview_canvas.bind("<B3-Motion>", on_pan_composite)
preview_canvas.bind("<ButtonRelease-3>", on_drag_release_composite)
right_arrow_button = ttk.Button(
    preview_controls_frame, text="▶", command=next_image, width=2)
right_arrow_button.pack(side=tk.LEFT, padx=5, pady=5, anchor='center')
vertical_buttons_frame = tk.Frame(preview_controls_frame)
vertical_buttons_frame.pack(side=tk.LEFT, padx=(10, 0), pady=0, anchor='n')
toggle_bg_button = ttk.Button(
    vertical_buttons_frame, text="Toggle Alpha", command=toggle_preview_background, width=15)
toggle_bg_button.pack(side=tk.TOP, pady=(5, 2))
composite_view_button = ttk.Button(
    vertical_buttons_frame, text="Composite View", command=toggle_composite_mode, width=15)
...
texture_label = tk.Label(
    texture_info_frame, text="No texture loaded", font=('Helvetica', 9), anchor='w')
texture_label.pack(side=tk.LEFT, padx=5)
image_dimensions_label = tk.Label(
    texture_info_frame, text=" ", font=('Helvetica', 10), anchor='e')
...
status_label = tk.Label(bottom_frame, text="Ready",
                        anchor=tk.W, fg="blue", font=('Helvetica', 10))
status_label.pack(side=tk.LEFT, padx=0)
file_path_label = tk.Label(
    bottom_frame, text="File: None", anchor=tk.W, font=('Helvetica', 9))
file_path_label.pack(side=tk.LEFT, padx=10)
internal_name_label = tk.Label(
    bottom_frame, text="Internal Name: Not Loaded", anchor=tk.E, font=('Helvetica', 10))
internal_name_label.pack(side=tk.RIGHT, padx=0)
import_button = ttk.Button(
    buttons_frame, text="IMPORT", command=import_texture, width=10)
import_button.pack(pady=2)
export_button = ttk.Button(
    buttons_frame, text="EXPORT", command=export_selected_file, width=10)
export_button.pack(pady=2)
save_button_main = ttk.Button(
    buttons_frame, text="SAVE", command=save_file, width=10)
